...
.DESCRIPTION
    ...
    (-App codex|opencode|traycer|t3code|grokbot|herdr). Builds to staging and swaps in place only while the
    patched "(RTL)" copy is closed.
.PARAMETER Force
    Rebuild even if the app version has not changed.
#>
[CmdletBinding()]
param([ValidateSet('codex','opencode','traycer','t3code','grokbot','herdr')][string]$App = 'codex', [switch]$Force, [switch]$AllowExternalNodeFallback)
