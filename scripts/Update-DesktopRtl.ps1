...
.DESCRIPTION
    ...
    (-App codex|opencode|traycer|t3code|grokbot). Builds to staging and swaps in place only while the
    patched "(RTL)" copy is closed.
.PARAMETER Force
    Rebuild even if the app version has not changed.
#>
[CmdletBinding()]
param([ValidateSet('codex','opencode','traycer','t3code','grokbot')][string]$App = 'codex', [switch]$Force, [switch]$AllowExternalNodeFallback)
