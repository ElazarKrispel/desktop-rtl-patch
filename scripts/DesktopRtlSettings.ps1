...
# to edit (codex|opencode|traycer|t3code|grokbot|herdr). The two GLOBAL toggles live
# in the unified tray, not here.
#
# -SelfTest builds the form without showing it (headless construction check).

param([ValidateSet('codex', 'opencode', 'traycer', 't3code', 'grokbot', 'herdr')][string]$App = 'codex', [switch]$NoRelaunch, [switch]$SelfTest)
