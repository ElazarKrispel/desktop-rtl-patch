...
function Get-RtlHebrewError([string]$msg) {
    ...
    switch -Regex ($msg) {
        ...
        '^\[UNSUPPORTED\]' { "גרסה זו של $appName משתמשת במבנה פנימי חדש שהכלי עדיין אינו תומך בו. לא בוצע שינוי. יש לעדכן את כלי ה-RTL כאשר תצא תמיכה למבנה זה." ; break }
        ...
        '^\[AV\]'      {
            $stage = try { $script:ActiveProfile.Staging } catch { $null }
            $base = "הפעולה נחסמה כנראה על ידי האנטי-וירוס (Windows Defender). הכלי עורך רק עותק מקומי של $appName ואינו נוגע במקור."
            if ($stage) {
                "$base`r`n`r`nאפשר לשחזר/לאשר את הפריט שסומן בממשק האנטי-וירוס, או להוסיף חריגה לתיקייה הבאה (הרצה ב-PowerShell כמנהל) ואז לנסות שוב:`r`n    Add-MpPreference -ExclusionPath '$stage'"
            } else {
                "$base אפשר לאשר זמנית או להוסיף חריגה ולנסות שוב."
            }
            break
        }
        ...
        '^\[PROFILE\]' { 'שגיאה פנימית בבחירת האפליקציה. ודא/י שבחרת אפליקציה מרשימת האפליקציות הנתמכות ונסה/י שוב.' ; break }
        '^\[PARTIAL\]' { "חלק מהקבצים לא הוסרו (אולי נעולים על ידי אנטי-וירוס, סייר הקבצים או תהליך פתוח). סגור/י אותם או הפעל/י מחדש את המחשב ונסה/י שוב. פרטים:`r`n`r`n$($msg -replace '^\[PARTIAL\]\s*','')" ; break }
        '^\[SOURCEGONE\]' { "$appName המקורי הוסר מהמחשב. עותק ה-RTL עדיין מותקן; אפשר להסיר אותו דרך ""הסר התקנה"", או להתקין מחדש את $appName." ; break }
